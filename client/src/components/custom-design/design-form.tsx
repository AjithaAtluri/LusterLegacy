...
export default function DesignForm({ onFormChange, formState }: DesignFormProps) {
  ...
  useEffect(() => {
    if (formState?.imageDataUrl && !previewUrl && uploadedImages.length === 0) {
      console.log("DesignForm - Received inspiration image from parent component:", formState.imageDataUrl);
      
      // Basic direct approach - just create an image element to display
      try {
        ...
        // Handle success
        img.onload = () => {
          console.log("DesignForm - Successfully loaded inspiration image:", img.width, "x", img.height);
          
          // Create a unique file name
          const fileName = "inspiration-" + Date.now() + ".jpg";
          
          // Create a fake file for the upload component
          const fakeFile = new File(
            [new ArrayBuffer(1)],  // Minimal placeholder, will be replaced by the actual image
            fileName,
            { type: "image/jpeg" }
          );
          
          // Create a preview URL directly from the source
          const newPreviewUrl = img.src;
          
          // Update component state with the image
          setUploadedImage(fakeFile);
          setUploadedImages([fakeFile]);
          setPreviewUrl(newPreviewUrl);
          setPreviewUrls([newPreviewUrl]);
          
          console.log("DesignForm - Set preview image directly from source");
        };
        
        // Handle errors
        img.onerror = (err) => {
          console.error("DesignForm - Failed to load inspiration image:", err);
        };
        
        // Set the source - this should work for both URLs and data URLs
        img.src = formState.imageDataUrl;
        
        // For URLs that might have CORS issues
        img.crossOrigin = "anonymous";
      } catch (error) {
        console.error("DesignForm - Error in inspiration image handling:", error);
      }
    }
